# FRT Projects & Partners

FRT Memory Vault is an open, verified infrastructure for AI long-term memory:
high-density storage, character-level lossless restoration (>=99%), and precision access.

We are looking for partners to run joint benchmarks on real AI/log/text workloads.

## FRT Projects (with releases)

| Project | Repository | Releases |
|---|---|---|
| FRT-Everitas-AI | https://github.com/januspater630/FRT-Everitas-AI | 5 |
| FRT-CFNS | https://github.com/januspater630/FRT-CFNS | 2 |
| FRT-Cosmic-Encryption | https://github.com/januspater630/FRT-Cosmic-Encryption | 1 |
| Janus-Vault | https://github.com/januspater630/Janus-Vault | 1 |
| FRT-Memory-Vault | https://github.com/januspater630/FRT-Memory-Vault | 1 |

## Potential Partners

### International

- Oracle — https://github.com/oracle
- Google Cloud — https://github.com/GoogleCloudPlatform
- Microsoft Azure — https://github.com/Azure
- Amazon Web Services — https://github.com/aws
- Snowflake — https://github.com/snowflakedb
- Databricks — https://github.com/databricks

### China

- JuiceFS — https://github.com/juicedata
- Tencent Cloud — https://github.com/TencentCloud
- Huawei Cloud — https://github.com/huaweicloud
- Alibaba Cloud — https://github.com/aliyun
- Baidu AI Cloud — https://github.com/baidubce
- MinIO — https://github.com/minio

## How to reach us

Open an issue in this repository, or contact us via the GitHub discussions.
